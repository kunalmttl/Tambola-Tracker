export const RULES = [
  ...
  },
  {
    ...
    checkFn: (ticket, calledNumbers) => {
      ...
      return nonZeros.length > 0 && nonZeros.every(n => calledSet.has(n));
    }
  },
  {
    id: 'MIDDLE_ROW',
  ...
  },
  {
    ...
    checkFn: (ticket, calledNumbers) => {
      ...
      const corners = [topZeros[0], topZeros[4], bottomZeros[0], bottomZeros[4]];
  ...
  },
  {
    ...
    checkFn: (ticket, calledNumbers) => {
      ...
      return targetNumbers.length > 0 && targetNumbers.every(n => calledSet.has(n));
    }
  },
  {
    id: 'PAKISTAN',
  ...
  },
  {
    id: 'STAR',
    name: 'Star',
    description: '4 corners plus the center number (3rd number of middle row)',
    defaultEnabled: false,
    checkFn: (ticket, calledNumbers) => {
      const calledSet = new Set(calledNumbers);
      const topZeros = ticket.grid[0].filter(n => n !== 0);
      ...
      if (topZeros.length < 5 || midZeros.length < 5 || bottomZeros.length < 5) return false;
      const starNumbers = [topZeros[0], topZeros[4], midZeros[2], bottomZeros[0], bottomZeros[4]];
  ...
  },
  {
    ...
    checkFn: (ticket, calledNumbers) => {
      ...
      for (const row of ticket.grid) {
        ...
        if (nonZeros.length > 0 && nonZeros.every(n => calledSet.has(n))) {
          return true;
        }
      }
      return false;
